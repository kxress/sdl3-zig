...
The supported path is source distribution only. It builds the pinned SDL source with the Emscripten
CMake toolchain, compiles the Zig consumer to a relocatable WebAssembly object, and lets `emcc`
perform the final runtime link. The fixture stages HTML, JavaScript glue, WebAssembly, and a preload
data file, then executes the generated JavaScript with Node:
...
The fixture uses emsdk 6.0.5 for reproducible validation. The active SDK must expose `EMSDK`; the
task is skipped when that variable is absent. The direct build inputs are:

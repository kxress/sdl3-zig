...
pub const Mixer = struct {
    ...
    /// Free a mixer.
    ///
    /// If this mixer was created with createMixerDevice(), this function will also close the audio device and call sdl.quitSubSystem(sdl.InitFlags.audio).
...
/// You can choose a specific device ID to open, following SDL's usual rules, but often the correct choice is to specify SDL_AUDIO_DEVICE_DEFAULT_PLAYBACK (C macro) and let SDL figure out what device to use (and seamlessly transition you to new hardware if the default changes).
/// Only playback devices make sense here. Attempting to open a recording device will fail.
/// This will call sdl.init(sdl.InitFlags.audio) internally; it's safe to call sdl.init() before this call, too, if you intend to enumerate audio devices to choose one to open here.
...
///
/// - **Parameters:**
///   - `devid`: the device to open for playback, or SDL_AUDIO_DEVICE_DEFAULT_PLAYBACK (C macro) for the default.
...
///
/// If you are linking to SDL_mixer dynamically, then it is possible that the current version will be different than the version you compiled against. This function returns the current version, while SDL_MIXER_VERSION (C macro) is the version you compiled with.
/// This function may be called safely at any time, even before init().
///
/// - **Returns:** the version of the linked library.
/// - **Since:** This function is available since SDL_mixer 3.0.0.
/// - **See also:** SDL_MIXER_VERSION (C macro)
